...
VALUES = np.array([100, 305, 333, 563, 950, 10000, -100, -305, -333, -563, -950, -10000])

class Silverman45ChessBoard(AbstractChessBoard):
    ...
    def _populate_board(self):
        ...
        black_pawns = [Pawn(PieceColor.BLACK, (-1, -1), PAWN_VALUE) for _ in range(self.width)]
        black_rooks = [Rook(PieceColor.BLACK, (-1, -1), ROOK_VALUE) for _ in range(2)]
        ...
        self.get((0, 0)).push(black_rooks[0])
        self.get((0, 1)).push(black_queen)
        self.get((0, 2)).push(black_king)
        self.get((0, 3)).push(black_rooks[1])

        white_pawns = [Pawn(PieceColor.WHITE, (-1, -1), PAWN_VALUE) for _ in range(self.width)]
        white_rooks = [Rook(PieceColor.WHITE, (-1, -1), ROOK_VALUE) for _ in range(2)]
        ...
        self.get((4, 0)).push(white_rooks[0])
        self.get((4, 1)).push(white_queen)
        self.get((4, 2)).push(white_king)
        self.get((4, 3)).push(white_rooks[1])
    ...
    def get_white_utility(self):
        return np.sum(np.tensordot(VALUES, self.state_vector(), axes=([0], [2])))/1000.0
    
    def get_black_utility(self):
        return -self.get_white_utility()
    ...
    def push(self, action: AbstractChessAction, check_for_check=True):
        ...
        if type(agent) == Pawn and agent.position[0] in [0, self.height - 1]:
            if AbstractActionFlags.PROMOTE_ROOK in action.modifier_flags:
                self.get(to_pos).pop()
    ...
    def condensed_action(self, a):
        return str(a.from_pos[0]) + str(a.from_pos[1]) + str(a.to_pos[0]) + str(a.to_pos[1])

    def condensed_legal_actions(self):
        return [self.condensed_action(a) for a in self.legal_actions()]
    ...
    def state_representation(self): # Make this more efficient
        rep = ""
