...
from minichess.games.gardner.action_reference import ID_TO_ACTION, ACTION_TO_ID, ID_TO_ACTION_BY_TILE, ID_TO_ACTION_BY_TILE

import numpy as np

class GardnerChessAction(AbstractChessAction):

    def __init__(self, agent: AbstractChessPiece, from_pos: tuple, to_pos: tuple, captured_piece: AbstractChessPiece = None, modifier_flags: List[AbstractActionFlags] = None, len_action_space = 1225):
        super().__init__(agent, from_pos, to_pos, captured_piece, modifier_flags, len_action_space = len_action_space)
    ...
    def encode(self) -> np.array:
        ...
        onehot = np.zeros(self.LEN_ACTION_SPACE)
        onehot[idx] = 1
